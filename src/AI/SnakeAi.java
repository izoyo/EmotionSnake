...
public class SnakeAi {
    ...
    public int play1(Snake s, Node f) {
        ...
        while (!q.isEmpty()) {
            ...
            if (!s.getMap().contains(up.toString()) && !vis.contains(up.toString())
                    && up.getX() <= Snake.map_sizeX && up.getX() >= Snake.size
                    && up.getY() <= Snake.map_sizeY && up.getY() >= Snake.size) {
                q.add(up);
                vis.add(up.toString());
                path.put(up.toString(), n.toString());
            }
            if (!s.getMap().contains(right.toString()) && !vis.contains(right.toString())
                    && right.getX() <= Snake.map_sizeX && right.getX() >= Snake.size
                    && right.getY() <= Snake.map_sizeY && right.getY() >= Snake.size) {
                q.add(right);
                vis.add(right.toString());
                path.put(right.toString(), n.toString());
            }
            if (!s.getMap().contains(down.toString()) && !vis.contains(down.toString())
                    && down.getX() <= Snake.map_sizeX && down.getX() >= Snake.size
                    && down.getY() <= Snake.map_sizeY && down.getY() >= Snake.size) {
                q.add(down);
                vis.add(down.toString());
                path.put(down.toString(), n.toString());
            }
            if (!s.getMap().contains(left.toString()) && !vis.contains(left.toString())
                    && left.getX() <= Snake.map_sizeX && left.getX() >= Snake.size
                    && left.getY() <= Snake.map_sizeY && left.getY() >= Snake.size) {
                q.add(left);
    ...
    public int play2(Snake snake, Node f) {
        ...
        for (int i = 0; i < snake.getS().size(); i++) {
            Node n = snake.getS().get(i);// 指针
            virSnake.getS().add(new Node(n.getX(), n.getY(), n.getFace()));
        }
//        virSnake.setS((ArrayList<Node>) snake.getS().clone()); //也是指针浅复制
        ...
        if (realGoTofoodDir != -1) {
            ...
            while (!virSnake.getFirst().toString().equals(f.toString())) {
                virSnake.move(play1(virSnake, f));// 这里怎么影响snake的？？
            }
            //虚拟蛇到尾巴去的方向
        ...
        while (!openList.isEmpty()) {
            ...
            for (Node n : temp) {
                ...
                if (s.getMap().contains(n.toString()) || closeList.contains(n)
                        || n.getX() > Snake.map_sizeX || n.getX() < Snake.size
                        || n.getY() > Snake.map_sizeY || n.getY() < Snake.size)
                    continue;
